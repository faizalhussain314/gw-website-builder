...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchStreamContent(payload, processedSelectors) {
    ...
    try {
      ...
      if (currentIndex < processedSelectors.length && result.trim().length > 0) {
        const selectorData = processedSelectors[currentIndex];
  ...
  window.addEventListener("message", async function (event) {
    if (event.data.type === "start") {
      ...
      fetchStreamContent({
        ...
        services_provided: event.data.description || "At Ajay's restaurant, we offer a diverse menu of authentic Indian cuisine.",
...
<script>
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.type === "nonClickable") {
      ...
      console.log("Appended non-clickable content:", bodyContent);
    }

    // Handle business name update for both header and footer
    if (event.data.type === "businessName") {
      console.log("Handling businessName event");

      // If the logo has already been updated, skip creating the span
      if (logoUpdated) {
        console.log("Logo was updated; skipping business name span creation.");
        return;
      }

      const businessName = event.data.text;
      updateLogoText("#template-logo", businessName);
      updateLogoText("#footer-logo", businessName);
    }

    // Handle logo change for both header and footer
    if (event.data.type === "changeLogo") {
      console.log("Handling changeLogo event");
  ...
  function updateLogoText(containerSelector, businessName) {
    ...
    if (existingText) {
      console.log(`Removing existing #logo-text in ${containerSelector}`);
      existingText.remove();
    }

    if (logoElement) {
      console.log(`Clearing logo image source in ${containerSelector}`);
      logoElement.src = "";
      logoElement.alt = "";
    ...
    const logoContainer = parentElement.querySelector("div > div > a > div > div");
    if (logoContainer) {
      logoContainer.appendChild(spanElement);
      console.log(`Appended new #logo-text span in ${containerSelector}:`, spanElement);
    } else {
  ...
  function updateLogoImage(containerSelector, logoUrl) {
    ...
    const existingText = parentElement.querySelector("#logo-text");
    if (existingText) {
      console.log(`Removing existing #logo-text during logo change in ${containerSelector}`);
      existingText.remove();
    }
    ...
    const logoElement = parentElement.querySelector("img:last-of-type");
    if (logoElement) {
      console.log(`Updating logo image in ${containerSelector} with new URL:`, logoUrl);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCSSVariable(variable, value) {
    ...
    if (!isUpdated) {
      ...
      console.log("Design changed using the root element:", variable, value);
      return true;
    }
    return false;
  }
  ...
  function updateCSSVariables(data) {
    if (data.type === "changeFont") {
      ...
      if (primary && secondary) {
        ...
        updateCSSVariable('--e-global-typography-secondary-font-family', secondary);
      } else if (primary) {
        updateCSSVariable('--e-global-typography-primary-font-family', primary);
      }
