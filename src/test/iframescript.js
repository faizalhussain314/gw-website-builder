...
function updateLogoText(containerSelector, businessName) {
  ...
  if (logoContainer) {
    ...
    console.log(
      `Appended new #logo-text span in ${containerSelector}:`,
      spanElement
    );
  } else {
...
function updateLogoImage(containerSelector, logoUrl) {
  ...
  if (existingText) {
    console.log(
      `Removing existing #logo-text during logo change in ${containerSelector}`
    );
    existingText.remove();
  }
  ...
  if (logoElement) {
    console.log(
      `Updating logo image in ${containerSelector} with new URL:`,
      logoUrl
    );
