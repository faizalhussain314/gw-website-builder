<?php
/**
 * GravityWrite eXtended RSS file parser implementations
 *
...
/**
 * GravityWrite Importer class for managing parsing of WXR files.
 */
class GW_Parser {
	function parse( $file ) {
		// Attempt to use proper XML parsers first
		if ( extension_loaded( 'simplexml' ) ) {
			$parser = new GW_Parser_SimpleXML;
		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . __( 'There was an error when reading this WXR file', 'gravitywrite-importer' ) . '</strong><br />';
			echo __( 'Details are shown above. The importer will now try again with a different parser...', 'gravitywrite-importer' ) . '</p>';
		}

		// use regular expressions if nothing else available or this is bad XML
		$parser = new GW_Parser_Regex;
		return $parser->parse( $file );
	}
}

/**
 * GravityWrite SimpleXML parser class
 */
class GW_Parser_SimpleXML {
        function parse( $file ) {
            $authors    = array();
            $posts      = array();
            $categories = array();
            ...
            if ( ! $success || isset( $dom->doctype ) ) {
                return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'gravitywrite-importer' ), libxml_get_errors() );
            }
            ...
            if ( ! $wxr_version ) {
                return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'gravitywrite-importer' ) );
            }
